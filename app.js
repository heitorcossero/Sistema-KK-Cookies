...
const isMesAtual = (dataIso) => {
  if (!dataIso) return false;
  const d = new Date(dataIso); const hoje = new Date();
...
window.reverterLancamento = async (id) => {
  ...
  try {
    if (h.tipo === 'compra') {
      ...
      if (it) { it.quantidade -= h.quantidade; await sincronizar('itens', it); }
    } else if (h.tipo === 'saida') {
      const it = state.itens.find(i => i.id === h.itemId);
      if (it) { it.quantidade += h.quantidade; await sincronizar('itens', it); }
    } else if (h.tipo === 'producao') {
      for (const d of (h.detalhesIngredientes || [])) {
        const it = state.itens.find(i => i.id === d.itemId);
        if (it) { it.quantidade += d.quantidade; await sincronizar('itens', it); }
      }
    } else if (h.tipo === 'congelado') {
      state.congelados[h.receitaId] = (state.congelados[h.receitaId] || 0) - h.quantidade;
      await sincronizar('congelados', null);
    }
    state.historico = state.historico.filter(x => x.id !== id);
    await sincronizar('historico', null, id);
    renderizar(); toast("Lançamento revertido!");
  } catch (err) { toast("Erro ao reverter.", true); }
};
...
window.toggleStatus = async (id, campo) => {
  ...
  if (enc) {
    ...
    if (campo === 'pago') {
      ...
      if (cli) { cli.ultimaConversa = new Date().toISOString(); await sincronizar('clientes', cli); }
    }
    await sincronizar('encomendas', enc);
    renderizar();
  }
};
...
window.excluir = async (tabela, id) => {
  ...
  if (tabela === 'congelados') { delete state.congelados[id]; await sincronizar('congelados', null); }
  else { state[tabela] = state[tabela].filter(x => x.id !== id); await sincronizar(tabela, null, id); }
  renderizar(); toast("Removido.");
};
...
function renderizar() {
  try {
    ...
    if (subtitle) {
      const status = supabase ? "✅ Conectado" : "💾 Offline";
      subtitle.innerHTML = `${status} - ${getNomeMesAtual()}`;
...
async function init() {
  ...
  document.getElementById("form-entrada").onsubmit = async (e) => {
    ...
    if (item && qtd > 0) {
      ...
      await sincronizar('itens', item); await sincronizar('historico', state.historico[0]);
  ...
  document.getElementById("form-congelados").onsubmit = async (e) => {
    ...
    if (recId && qtd > 0) {
      ...
      await sincronizar('congelados', null); await sincronizar('historico', state.historico[0]);
